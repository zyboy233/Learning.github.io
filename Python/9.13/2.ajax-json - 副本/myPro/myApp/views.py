...
class DuanZiView(View):
    def get(self,request):
        ...
        duanzi_list = requests.get(url).json()['data']
        myList = list(range(1,11))
        paginator = Paginator(duanzi_list,len(duanzi_list))
        page = int(page)
        return render(request,'duanzi.html',{'page':page,'paginator':paginator,'list':myList})
    def post(self,request):
