...
# https://search.51job.com/list/040000,000000,0000,00,9,99,python,2,1.html
...
class Job(object):
    ...
    def get_content_with_code(self,code):
        pattern = re.compile(r'<p class="t1 ">.*?<span>.*?<a.*?title="(.*?)".*?>.*?</a>.*?<span class="t2">.*?<a.*?title="(.*?)".*?>.*?</a>',re.S)
    ...
    def get_code_with_url(self,indexPage):
        url = self.baseUrl + self.get_city_code_with_name(
            self.cityName) + ',000000,0000,00,9,99,' + self.jobName + ',2,' + str(indexPage) + '.html'
